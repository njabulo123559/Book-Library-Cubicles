...
def homepage(request):
    all_location = Cubicle.objects.values_list('location','id').distinct().order_by()
    if request.method =="POST":
        try:
            print(request.POST)
            hotel = Cubicle.objects.all().get(id=int(request.POST['search_location']))
            rr = []
            ...
            if len(room) == 0:
                messages.warning(request,"Sorry No Cubicles Are Available on this time period")
